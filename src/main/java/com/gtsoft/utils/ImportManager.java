...
public class ImportManager implements Runnable {
	...
	public void doImport() throws Exception {
		...
		while( i.hasNext() ) {
			...
			m.setAnno( Integer.valueOf( c.getData().ymdString().substring(0,4) ) ) ;
			m.setData(c.getData());
...
//			int idCarico = -1 ;
			...
			Vector list = Carico.getCarichiDaScaricare( codRifiuto, Integer.valueOf( giorno.getAnno()) ) ;
...
//			Vector v = null ;
			...
			for ( Iterator i = listScarichi.iterator() ; i.hasNext() ; ) {
				...
				scarico.setAnno( Integer.valueOf( c.getData().ymdString().substring(0,4) ) ) ;
...
//		if ( ! RifiutiFrame.separateFumi || RifiutiFrame.isFumi  || RifiutiFrame.isRifiuto342  ) {
			...
			for ( Iterator i = rifiuti.iterator() ; i.hasNext() ; ) {
				...
				if ( giacenza > 0 ) {
					...
					m = new Movimento() ;
					m.setCarico( c ) ;
					m.setAnno( Integer.valueOf(new FormattedDate().getAnno()) ) ;
					m.setData( c.getData());
