...
public class JRSumRifiuto implements JRDataSource {
    ...
    HashMap<String,HashMap> list = null ;
    ...
    public JRSumRifiuto( Vector l ) throws Exception {
        ...
        for ( Iterator i = l.iterator() ; i.hasNext() ; ) {
            ...
            if ( listCod.containsKey(r.getCodiceQuadrelli())) {
                ...
                if ( cCaricato == null ) {
                    cCaricato = Double.valueOf(0) ;
                }
                if ( cTrattato == null ) {
                    cTrattato = Double.valueOf(0) ;
                }
            } 
            else {
                values = new HashMap() ;
            ...
            if ( m.isScarico() || m.isParziale() ) {
                cTrattato = Double.valueOf( cTrattato.doubleValue() + m.getPeso().doubleValue() ) ;
            }
            else {
                cCaricato = Double.valueOf( cCaricato.doubleValue() + m.getPeso().doubleValue() ) ;
            }
        ...
        for ( Iterator iRifiuti = list.values().iterator() ; iRifiuti.hasNext() ; ) {
            ...
            if ( sum > 0 )
                values.put(Fields.GIACENZA, Double.valueOf(cCaricato.doubleValue() - cTrattato.doubleValue()) ) ;
            else
	            iRifiuti.remove() ;
	            */
            values.put(Fields.GIACENZA, Double.valueOf(cCaricato.doubleValue() - cTrattato.doubleValue()) ) ;
