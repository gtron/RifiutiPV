...
public class stampaRegistri_jDialog extends JDialog {
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {
				    try {
				        ...
				        ReportManager.printPageNumber( 
				           Integer.parseInt( prepare_Da_jTextField.getText() ),
				           Integer.parseInt( prepare_num_jTextField.getText() )
				           );
				        setVisible(false);
				    }
				    catch (Exception ex) {
	...
	private Vector getDaStampare(boolean checkRange) throws Exception {
	    ...
	    Integer end = null ;
	    try {
	        start = new Integer( numStart_jTextField.getText() ) ;
	        end = new Integer( numEnd_jTextField.getText() ) ;
	    }
	    catch ( Exception e ) {
